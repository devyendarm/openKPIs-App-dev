...
export default function EventEditClient({ event, slug, canEdit }: EventEditClientProps) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error: updateError } = await (supabase.from(eventsTable) as any)
      ...
      const syncResponse = await fetch(`/api/events/${event.id}/sync-github`, {
        method: 'POST',
    ...
    return (
      <main style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--ifm-color-emphasis-700)' }}>
          You do not have permission to edit this event. Sign in with the owner account if you believe this is an error.
        </p>
