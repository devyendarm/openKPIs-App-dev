...
const TYPE_LABEL = 'Dashboard';
const DATA_LAYER_DIR = 'dashboards';
...
export async function POST(
  ...
  try {
    ...
    const yamlContent = generateYAML(dashboard);
    const fileName = `${dashboard.slug || dashboard.name || dashboard.id}.yml`;
    ...
    if (dashboard.github_pr_number) {
      try {
        const { data: pr } = await octokit.pulls.get({
          owner: GITHUB_OWNER,
          repo: GITHUB_CONTENT_REPO,
          pull_number: dashboard.github_pr_number,
        });
        existingPR = pr;
        branchName = pr.head.ref;
      } catch (e) {
        branchName = `${action}-${TYPE_LABEL.toLowerCase()}-${dashboard.slug || dashboard.id}-${Date.now()}`;
      }
    } else {
      branchName = `${action}-${TYPE_LABEL.toLowerCase()}-${dashboard.slug || dashboard.id}-${Date.now()}`;
    }
    ...
    const statusSuffix = dashboard.status === 'published' ? ' (Published)' : '';
    ...
    const { data: commitData } = await octokit.repos.createOrUpdateFileContents({
      ...
      repo: GITHUB_CONTENT_REPO,
      path: filePath,
      message: `${actionLabel} ${TYPE_LABEL}: ${dashboard.name || dashboard.slug}${statusSuffix}`,
      ...
      author: {
        ...
        email: `${dashboard.created_by}@users.noreply.github.com`,
      },
    });
    ...
    if (existingPR) {
      prData = existingPR;
      const prBody = `**Contributed by**: @${dashboard.created_by}\n\n**Action**: ${action}\n**Type**: ${TYPE_LABEL}\n**Status**: ${dashboard.status}\n\n---\n\n${dashboard.description || 'No description provided.'}`;
      await octokit.pulls.update({
        owner: GITHUB_OWNER,
        repo: GITHUB_CONTENT_REPO,
        pull_number: existingPR.number,
        title: `${actionLabel} ${TYPE_LABEL}: ${dashboard.name || dashboard.slug}${statusSuffix || ' (Draft)'}`,
        body: prBody,
      });
    } else {
      const { data: newPR } = await octokit.pulls.create({
        owner: GITHUB_OWNER,
        repo: GITHUB_CONTENT_REPO,
        title: `${actionLabel} ${TYPE_LABEL}: ${dashboard.name || dashboard.slug}${dashboard.status === 'draft' ? ' (Draft)' : ' (Ready for Review)'}`,
        head: branchName,
        base: 'main',
        body: `**Contributed by**: @${dashboard.created_by}\n\n**Action**: ${action}\n**Type**: ${TYPE_LABEL}\n**Status**: ${dashboard.status}\n\n---\n\n${dashboard.description || 'No description provided.'}`,
