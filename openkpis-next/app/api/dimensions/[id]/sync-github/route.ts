...
const TYPE_LABEL = 'Dimension';
const DATA_LAYER_DIR = 'dimensions';
...
export async function POST(
  ...
  try {
    ...
    const yamlContent = generateYAML(dimension);
    const fileName = `${dimension.slug || dimension.name || dimension.id}.yml`;
    ...
    if (dimension.github_pr_number) {
      try {
        const { data: pr } = await octokit.pulls.get({
          owner: GITHUB_OWNER,
          repo: GITHUB_CONTENT_REPO,
          pull_number: dimension.github_pr_number,
        });
        existingPR = pr;
        branchName = pr.head.ref;
      } catch (e) {
        branchName = `${action}-${TYPE_LABEL.toLowerCase()}-${dimension.slug || dimension.id}-${Date.now()}`;
      }
    } else {
      branchName = `${action}-${TYPE_LABEL.toLowerCase()}-${dimension.slug || dimension.id}-${Date.now()}`;
    }
    ...
    const statusSuffix = dimension.status === 'published' ? ' (Published)' : '';
    ...
    const { data: commitData } = await octokit.repos.createOrUpdateFileContents({
      ...
      repo: GITHUB_CONTENT_REPO,
      path: filePath,
      message: `${actionLabel} ${TYPE_LABEL}: ${dimension.name || dimension.slug}${statusSuffix}`,
      ...
      author: {
        name: dimension.created_by,
        email: `${dimension.created_by}@users.noreply.github.com`,
      },
    });
    ...
    if (existingPR) {
      prData = existingPR;
      const prBody = `**Contributed by**: @${dimension.created_by}\n\n**Action**: ${action}\n**Type**: ${TYPE_LABEL}\n**Status**: ${dimension.status}\n\n---\n\n${dimension.description || 'No description provided.'}`;
      await octokit.pulls.update({
        owner: GITHUB_OWNER,
        repo: GITHUB_CONTENT_REPO,
        pull_number: existingPR.number,
        title: `${actionLabel} ${TYPE_LABEL}: ${dimension.name || dimension.slug}${statusSuffix || ' (Draft)'}`,
        body: prBody,
      });
    } else {
      const { data: newPR } = await octokit.pulls.create({
        owner: GITHUB_OWNER,
        repo: GITHUB_CONTENT_REPO,
        title: `${actionLabel} ${TYPE_LABEL}: ${dimension.name || dimension.slug}${dimension.status === 'draft' ? ' (Draft)' : ' (Ready for Review)'}`,
        head: branchName,
        base: 'main',
        body: `**Contributed by**: @${dimension.created_by}\n\n**Action**: ${action}\n**Type**: ${TYPE_LABEL}\n**Status**: ${dimension.status}\n\n---\n\n${dimension.description || 'No description provided.'}`,
