...
export async function POST(
  ...
  try {
    ...
    const yamlContent = generateYAML(event);
    const fileName = `${event.slug || event.name || event.id}.yml`;
    ...
    if (event.github_pr_number) {
      try {
        ...
        branchName = `${action}-${TYPE_LABEL.toLowerCase()}-${event.slug || event.id}-${Date.now()}`;
      }
    } else {
      branchName = `${action}-${TYPE_LABEL.toLowerCase()}-${event.slug || event.id}-${Date.now()}`;
    }
    ...
    const statusSuffix = event.status === 'published' ? ' (Published)' : '';
    ...
    const { data: commitData } = await octokit.repos.createOrUpdateFileContents({
      ...
      repo: GITHUB_CONTENT_REPO,
      path: filePath,
      message: `${actionLabel} ${TYPE_LABEL}: ${event.name || event.slug}${statusSuffix}`,
      ...
      author: {
        ...
        email: `${event.created_by}@users.noreply.github.com`,
      },
    });
    ...
    if (existingPR) {
      prData = existingPR;
      const prBody = `**Contributed by**: @${event.created_by}\n\n**Action**: ${action}\n**Type**: ${TYPE_LABEL}\n**Status**: ${event.status}\n\n---\n\n${event.description || 'No description provided.'}`;
      await octokit.pulls.update({
        owner: GITHUB_OWNER,
        repo: GITHUB_CONTENT_REPO,
        pull_number: existingPR.number,
        title: `${actionLabel} ${TYPE_LABEL}: ${event.name || event.slug}${statusSuffix || ' (Draft)'}`,
        body: prBody,
      });
    } else {
      const { data: newPR } = await octokit.pulls.create({
        owner: GITHUB_OWNER,
        repo: GITHUB_CONTENT_REPO,
        title: `${actionLabel} ${TYPE_LABEL}: ${event.name || event.slug}${event.status === 'draft' ? ' (Draft)' : ' (Ready for Review)'}`,
        head: branchName,
        base: 'main',
        body: `**Contributed by**: @${event.created_by}\n\n**Action**: ${action}\n**Type**: ${TYPE_LABEL}\n**Status**: ${event.status}\n\n---\n\n${event.description || 'No description provided.'}`,
